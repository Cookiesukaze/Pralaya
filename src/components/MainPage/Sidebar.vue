<template>
  <div :class="isSidebarCollapsed ? 'w-20' : 'w-72'" class="fixed inset-y-0 z-50 flex flex-col transition-all duration-300"  style="background-color: #f3f3f9;" >
    <div class="flex grow flex-col gap-y-5 overflow-y-auto border-r border-themeBorderGrey125 px-4 sidebar-hide-scrollbar">
      <div class="flex h-16 shrink-0 items-center justify-between">
        <img v-if="!isSidebarCollapsed" class="h-12 w-auto" :src="company.logo" alt="Company logo" />

        <button @click="toggleSidebar"
                :class="[
                  'group flex items-center justify-center',
                  isSidebarCollapsed ? 'p-3' : 'px-4 py-2',
                  'rounded-md text-themeFontGrey hover:text-themeBlue'
                ]">
      ...
      <nav class="flex flex-1 flex-col overflow-hidden">
        <ul role="list" class="flex flex-1 flex-col gap-y-7 overflow-hidden">
          <li>
            <div v-if="!isSidebarCollapsed" class="text-xs font-semibold leading-6 text-themeFontGrey">你的图谱</div>
            <ul role="list" class="-mx-2 mt-2 space-y-1 list-none">
              <li v-for="graph in graphs" :key="graph.id">
                <a @click="graph.isAvailable !== null && graph.isAvailable !== 'NO' && selectGraph(graph.id)"
                   :class="[graph.current ? 'text-themeBlue' : 'text-themeFontGrey hover:text-themeBlue hover:bg-themeGrey25',
                  ...
                  <component
                      ...
                      aria-hidden="true"
                  />
                  <span v-if="!isSidebarCollapsed" :class="[graph.current ? 'text-themeBlue' : 'text-themeFontGrey group-hover:text-themeBlue']" class="truncate">{{ graph.name }}</span>
                </a>
              </li>
            </ul>
          </li>
      ...
      <div v-if="!isSidebarCollapsed" class="flex justify-center mt-auto mb-1 px-6">
        <button @click="createNewGraph" class="w-full border text-themeBlue rounded-md px-4 py-2 hover:bg-themeBlue25 theme-button">
          创建新的图谱
        </button>
      </div>

      <li class="-mx-6 mb-2 list-none">
        <a href="#" class="flex items-center gap-x-4 px-6 py-2 text-sm font-semibold leading-6 text-themeFontGrey hover:bg-themeGrey25">
