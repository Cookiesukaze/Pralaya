<template>
  <div :class="isSidebarCollapsed ? 'w-20' : 'w-72'" class="fixed inset-y-0 z-50 flex flex-col transition-all duration-300"  style="background-color: #768ab1;" >
    <div class="flex grow flex-col gap-y-5 overflow-y-auto  px-4 sidebar-hide-scrollbar">
      <div class="flex h-16 shrink-0 items-center justify-between">
        ...
        <button @click="toggleSidebar"
                :class="[
                  ...
                  'rounded-md text-white hover:text-themeBlue'
                ]">
          <ChevronDoubleLeftIcon v-if="!isSidebarCollapsed" class="h-6 w-6 shrink-0 text-white" />
          <ChevronDoubleRightIcon v-else class="h-6 w-6 shrink-0 text-white" />
        </button>
      </div>
      ...
      <nav class="flex flex-1 flex-col overflow-hidden">
        <ul role="list" class="flex flex-1 flex-col gap-y-7 overflow-hidden">
          <li>
            <div v-if="!isSidebarCollapsed" class="text-xs font-semibold leading-6 text-white">你的图谱</div>
            <ul role="list" class="-mx-2 mt-2 space-y-1 list-none">
              <li v-for="graph in graphs" :key="graph.id">
                <a @click="graph.isAvailable !== null && graph.isAvailable !== 'NO' && selectGraph(graph.id)"
                   :class="[graph.current ? 'text-white bg-themeGrey75' : 'text-white hover:text-themeBlue25 hover:bg-themeGrey75',
      ...
      <div v-if="!isSidebarCollapsed" class="flex justify-center mt-auto mb-1 px-6">
        <button @click="createNewGraph" class="w-full border text-themeBlue rounded-md px-4 py-2 hover:bg-themeBlue theme-button">
          创建新的图谱
        </button>
      </div>

      <li class="-mx-6 mb-2 list-none">
        <a href="#" class="flex items-center gap-x-4 px-6 py-2 text-sm font-semibold leading-6 text-white ">
