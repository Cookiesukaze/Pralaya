<template>
  <div class="flex justify-between items-center bg-white shadow px-6 py-4">
    <!-- 当前图表的名字 -->
    <div class="text-lg font-semibold">{{ selectedGraph.name }}</div>
    ...
    <div class="relative">
      ...
      <div v-if="isDropdownOpen" @mouseenter="cancelCloseDropdown" @mouseleave="scheduleCloseDropdown" class="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 py-4 z-10">
        <div class="px-4 py-2 text-sm">
          ...
          />
          <p class="text-center mt-2 text-gray-700">{{ selectedGraph.description }}</p>
          <button class="mt-4 w-full px-4 py-2 bg-indigo-600 text-white rounded-md text-sm shadow-sm hover:bg-indigo-700">
            编辑图谱
          </button>
        </div>
      </div>
